...
* ⚠️ WHAT IT BUYS WHERE THE STENCIL REFUSES IS A NUMBER AND NOT AN INTERVAL.
  ...
  and what a reader is told is unchanged: the curvature at a boundary is not
  negative definite, its smallest eigenvalue reading -2.5e-05 against a
  largest of 5.86, so `statmod_hyper_vcov()` still declines and the
  certificate still says `boundary`.
