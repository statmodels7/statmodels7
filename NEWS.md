...
* `statmod_penalized()` enumerates the penalized units of a model -- one
  ...
  need for a population value with shrunk deviations per group. The
  remaining nine call sites are reporting and hyperparameter estimation;
  they still key by term name, which is correct for every term that ships
  today and is what the rest of the sweep has to reach before a term with
  two penalties can be fitted end to end.
