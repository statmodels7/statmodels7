...
``` r
statmod_stat(fit, param, coefname, value = 0, test = "wald", type = "bayesian")
```
...
A list with `test`, `statistic`, `df`, `p.value`, and `converged`, which
is `NA` for the Wald statistic and the restricted fit's flag for the
other three.
