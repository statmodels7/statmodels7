...
for the printing alone; [`coef`](https://rdrr.io/r/stats/coef.html) and
the summary's own tables keep the names the fit was built with, which
are the ones another call can be indexed by.

ONE PIECE and not every piece they share: the term's own name is one,
and a set of coefficients that happen to agree further along – `r.1`,
`r.2` of a matrix column – would otherwise be left as the bare numbers.
