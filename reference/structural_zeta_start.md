...
zero, so it starts at a weak response instead.
...
- target:
  ...
  The response on the scale of the term's equation, as
  [`predictor_target()`](https://statmodels7.github.io/statmodels7/reference/predictor_target.md)
  returns it, or `NULL`. A term whose start is read off the data
  consumes it, the marginal break-point term's exact profile being the
  case; every other method ignores it through the dots.
