...
`tol` is 1e-2 instead of the geometric middle of the two groups: the two
ways of being wrong are not symmetric, and a certificate that says not
converged at a good point is visible and checkable where one that
...
is also `"unknown"` and never approximated: 2p refits to difference it
