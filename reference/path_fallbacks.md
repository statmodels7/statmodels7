...
`enet(x, n_lambda = 25, n_alpha = 5)` – so nothing here is reached for
them. What reaches it is a term that declares a kinked penalty without
offering an argument for the grid:
[`random`](https://statmodels7.github.io/modelterms7/reference/random.html)
...
geometrically over `1/min_ratio` – four decades – and wants that many
points to be smooth in. `other` serves an axis that spans one bounded
interval instead, \\\alpha\\ between the ridge and the lasso or a shape
over its useful range, and needs fewer; with a product every extra point
there multiplies the fits.
