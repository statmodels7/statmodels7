...
coefficients of a design block: there is no \\X'WX\\ to invert, the
...
chains the filter's own jacobian, which is the total derivative of the
predictor in those parameters – so a quasi-Newton method has everything
it needs.
