...
enough and sparse enough to be worth it
...
Those figures are the ones measured with Matrix's factorization cache
defeated.
...
refactorizes the same object measures a cache hit, 0.004 ms against
...
and the Hessian's pair loop among them, so a dense inverse of a sparse
matrix costs nothing sparsity could have kept. What the sparse factor
