...
[`term_continue`](https://statmodels7.github.io/modelterms7/reference/term_continue.html).
...
run over them from the term's own seed, which is what a caller means by
...
series. A frame with the response on some rows and not others is
rejected: the two readings differ, and picking one would answer a
...
over states, which past the data is a predictive distribution and not a
value.
