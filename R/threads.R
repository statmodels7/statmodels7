...
#'
#' @description
#' \code{t(A) \%*\% (w * B)}, through the package's threaded kernel where the
#' thread count and the measured work threshold allow it, and through
#' \code{crossprod(A * w, B)} -- exactly the expression it replaces --
#' everywhere else.
#'
#' @details
...
#' almost nothing.
#'
#' @param A,B Design blocks, \code{n x pa} and \code{n x pb}.
#' @param w The per-observation weights, length \code{n}.
#' @param threads The thread count, a plain integer.
#'
#' @return A \code{pa x pb} matrix.
#'
#' @keywords internal
...
#'
#' @description
#' \code{xtv()} is \code{as.numeric(crossprod(X, v))}, one dot product per
#' column; \code{wxsq()} is \code{as.numeric(crossprod(w, X^2))}, the column
#' curvatures of a working model, computed without materializing the
#' \code{n x p} square. Both run through the package's threaded kernels
#' above the same measured work gate as \code{\link{wcrossprod}} and through
...
#'
#' @param X A design block, \code{n x p}.
#' @param v,w Per-observation vectors of length \code{n}.
#' @param threads The thread count, a plain integer.
#'
#' @return A numeric vector of length \code{ncol(X)}.
#'
#' @keywords internal
...
#'
#' @description
#' \code{crossprod(A)} through the threaded kernel where the count and the
#' measured work gate allow it, and through \code{crossprod(A)} itself
#' everywhere else. What \code{fit_smooth()}'s subset route assembles at
